...
to only include those that are amicable to the RoboDecomposer+Classifier pipeline.
...
amicability = json.loads(Path('amicability.json').read_text())

chunk_size = 100_000
classifier = Classifier(amicability)
...
def process_chunk(chunk:str, filename:str, i:int, headers: List[str]):
    ...
    if sum(verdicts.acceptable):
        for key in ['N_synthons', 'synthon_amicability', 'weighted_robogroups']:
            df[key] = verdicts[key]
        txt = '\t'.join(map(str, headers)) + '\n'
        cols = ['SMILES', 'Identifier', 'synthon_amicability', 'N_synthons', 'weighted_robogroups']
