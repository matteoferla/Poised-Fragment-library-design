...
class Pipiteur:
    ...
    def __init__(self, order: int = 3,
        ...
        self.wanted = sorted(self.wanted)  # overrides the class variable, but hey
    ...
    @staticmethod
    def calc_size(min_d: int, max_d: int, resolution: float,order: int) -> int:
        """
        How many bins will there be?

        * order=3, min_d = 2, max_d = 8, resolution = 1. => 343 bins
        * order=3, min_d = 2, max_d = 8, resolution = 0.5 => 2,197 bins
        """
        return int((max_d - min_d) // resolution + 1) ** order